...
![img](java重点.assets/cfa08234073111c08f451749f64bde39.jpeg)
...
![img](java重点.assets/5f06bbf0993a5bdec7ecc87390717c67.jpeg)

**二、java中的IO模型**

* BIO：同步阻塞IO，在此种方式下，用户进程在发起一个IO操作以后，必须等待IO操作的完成，只有当真正完成了IO操作以后，用户进程才能运行
  * 线程阻塞：每个客户端连接分配一个线程，线程在等待数据或处理 I/O 时会挂起。
  * 一对一模型：一个连接对应一个线程。如果有 1000 个连接，就需要 1000 个线程。	

* NIO：同步非阻塞IO，在此种方式下，用户进程发起一个IO操作以后便可去做其它事情，但是用户进程需要时不时的询问IO操作是否就绪，这就要求用户进程不停的去询问，从而引入不必要的CPU资源浪费。
  * 线程非阻塞：线程发起 I/O 操作后立即返回，可以继续处理其他任务
  * 一对多模型：一个线程通过 Selector 管理多个连接（Channel），大幅减少线程数。
...
* 策略模式的优点
  ...
  - 解耦：算法的实现与使用分离，符合开闭原则（对扩展开放，对修改关闭）
...
**策略存到哪**：将所有策略实例存储在一个集合中，通常是 Map，以键值对的形式管理（键是策略的标识，值是策略对象）。
...
### 可以自定义一个java.lang.String类并被类加载吗
...
**1、手动实现深拷贝，在对象中实现 `clone()` 方法**

```java
List a, list b; aimport java.util.ArrayList;
import java.util.List;
...
public class DeepCopyExample {
    public static void main(String[] args) {
        List<Example> listA = new ArrayList<>();
        listA.add(new Example("A"));
        listA.add(new Example("B"));

        // 深拷贝
        List<Example> listB = new ArrayList<>();
        for (Example item : listA) {
            listB.add(item.clone());
        }

        // 修改 listB 的内容
        listB.get(0).setValue("C");

        System.out.println("List A: " + listA.get(0).getValue()); // 输出 A
        System.out.println("List B: " + listB.get(0).getValue()); // 输出 C
    }
}
```
...
​	通过将对象序列化为二进制数据流，然后反序列化为一个新的对象，达到深拷贝的目的
...
public class DeepCopyWithSerialization {
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        List<String> listB = (List<String>) in.readObject();

        listB.set(0, "C");

        System.out.println("List A: " + listA.get(0)); // 输出 A
        System.out.println("List B: " + listB.get(0)); // 输出 C
    }
}
```
...
[参考](https://blog.51cto.com/mjunz/13047483)
...
![G1原理—1.G1回收器的分区机制_G1_04](java重点.assets/format,webp.webp)
...
* 同时系统也一直在不断地运行，有时是高峰期，有时是低谷期

**改进**

距离本次预测越近的GC其影响权重就越高，比如已经发生了3次GC，现在要预测第4次GC。那么第一次权重是0.2，第二次权重是0.3，第三次GC的权重可能就是0.5。
